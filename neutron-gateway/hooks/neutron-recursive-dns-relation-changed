...
@hooks.hook('neutron-recursive-dns-relation-joined')
@hooks.hook('neutron-recursive-dns-relation-changed')
@hooks.hook('neutron-recursive-dns-relation-departed')
@hooks.hook('neutron-recursive-dns-relation-broken')
@restart_on_change(restart_map())
def config_recursive_dns():
    #recursion_ip_list = []
    #for relid in relation_ids('neutron-recursive-dns'):
    #    log("config_recursive_dns adding relid: {}".format(relation_get(rid=relid)))
    #    recursion_ip_list.append(relation_get(rid=relid)['recursion_ip'])
    #    log("config_recursive_dns adding relid: {}".format(recursion_ip_list))
    CONFIGS.write_all()
